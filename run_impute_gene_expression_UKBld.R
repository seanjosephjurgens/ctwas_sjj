...
weight <- paste0("~/cTWAS/data/GTEx_models/weights/eqtl/mashr/mashr_", tissue, ".db")
...
if (!file.exists(paste0("~/cTWAS/data/GTEx_models/weights_nolnc/mashr_", tissue, "_nolnc_subset_chunk", chunk, ".db"))){
  db <- dbConnect(sqlite, paste0("~/cTWAS/data/GTEx_models/weights_nolnc/mashr_", tissue, "_nolnc_subset_chunk", chunk, ".db"))
  dbWriteTable(db, "extra", extra_table)
  dbWriteTable(db, "weights", weights_table)
  dbDisconnect(db)
  
  weight_info_gz <- gzfile(paste0("~/cTWAS/data/GTEx_models/weights_nolnc/mashr_", tissue, "_nolnc_subset_chunk", chunk, ".txt.gz"), "w")
...
weight_subset <- paste0("~/cTWAS/data/GTEx_models/weights_nolnc/mashr_", tissue, "_nolnc_subset_chunk", chunk, ".db")
...
try(save(res, file=paste0(outputdir, '/', gwas_name, '_chunk', chunk, '_UKBld_Zresults.Rd')))
z_snp <- res$z_snp
try(save(z_snp, file=paste0(outputdir, '/', gwas_name, '_chunk', chunk, '_UKBld_Zsnp_results.Rd')))
z_gene <- res$z_gene
try(save(z_gene, file=paste0(outputdir, '/', gwas_name, '_chunk', chunk, '_UKBld_Zgene_results.Rd')))
